...
public class TaoSubtree implements Subtree {
	...
	// Every level greater than lastLevelToSave will be deleted when a path is
	// deleted
	...
	public void initRoot() {
		...
		// Check if we have more than one server, in which case we must initialize the
		// subtree
		if (numServers > 1) {
	 ...
	 */
	public long deleteChild(SubtreeBucket bucket, long pathID, boolean[] directions, int level, long minTime,
			Set<Long> pathReqMultiSet) {
		...
		TaoLogger.logDebug(
				"The current parent level is " + parentLevel + " and the lastLevelToSave is " + lastLevelToSave);
	...
	public void deleteNodes(long pathID, long minTime, Set<Long> pathReqMultiSet) {
		...
		synchronized (mRootLock) {
			...
			// NOTE: If root has a timestamp less than minTime, the the entire subtree
			// should be able to be deleted, and
