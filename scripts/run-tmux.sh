...
# launch servers
_tmux_process_command $SERVER_SESSION 0 'C-c' "./scripts/run-server.sh 0 2>&1 > server0.log"
_tmux_process_command $SERVER_SESSION 1 'C-c' "./scripts/run-server.sh 1 2>&1 > server1.log"
_tmux_process_command $SERVER_SESSION 2 'C-c' "./scripts/run-server.sh 2 2>&1 > server2.log"

echo 'Servers launched.'

# launch proxies
_tmux_process_command $PROXY_SESSION 0 'C-c' "./scripts/run-proxy.sh 0 2>&1 > proxy0.log"
_tmux_process_command $PROXY_SESSION 1 'C-c' "./scripts/run-proxy.sh 1 2>&1 > proxy1.log"
_tmux_process_command $PROXY_SESSION 2 'C-c' "./scripts/run-proxy.sh 2 2>&1 > proxy2.log"
...
_tmux_process_command $CLIENT_SESSION 0 'C-c' "./scripts/run-client.sh $num_clients $loadtest_length $rw_ratio $zipf $num_warmup $quorum_type 0 2>&1 > client.log"
